---
name: feedback
description: Report a bug or send feedback about the replit-migrate plugin
---

# Report replit-migrate feedback

File the user's report as a GitHub issue on `tyroneross/replit-migrate`. Issues are this plugin's
...
```bash
gh issue create --repo tyroneross/replit-migrate \
...
5. If `gh` is missing or unauthenticated, do not fail — print the URL so the user
   can open it in a browser: https://github.com/tyroneross/replit-migrate/issues/new
